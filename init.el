...
(with-eval-after-load 'eglot
  ...
  (add-to-list 'eglot-server-programs
               '(simpc-mode . ("clangd" "--background-index" "--clang-tidy")))
  (add-to-list 'eglot-server-programs
        		...
        		(let* ((file (buffer-file-name))
        		       ...
        		       (name (when file
        				...
        				(file-name-sans-extension file)))))
                          (if file
                              (if (member ext '("cpp" "cc" "hpp" "hh"))
                                  (format "g++ -Wall -std=c++20 -O2 -g -o \"%s\" \"%s\" && ./%s" name file name)
                                (format "gcc -Wall -O2 -g -o \"%s\" \"%s\" && ./%s" name file name))
                            "make -k")))))
...
(add-hook 'eglot-managed-mode-hook
          (lambda ()
            ...
            (unless (member major-mode
                            '(json-mode css-mode js-mode typescript-mode))
